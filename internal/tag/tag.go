...
// Pyroscope's `app{key=value,...}` grammar: braces, `=` and `,` are delimiters; quotes and
...
// SanitizeValue makes a value safe for Pyroscope's `app{key=value,...}` name; replacement is
// one-to-one so the value stays readable in the UI.
...
// A value opening with `{{` is always dynamic, so a parse failure is an error.
